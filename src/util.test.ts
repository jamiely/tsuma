...
import { add, chaikinSmoothing, distance, getIntersection, inBounds, magnitude, scale, setPoint, subtract, toUnit } from './util'
...
test('inBounds', () => {
  ...
  expect(inBounds({x: 1, y: 0}, bounds, noPadding)).toBe(false);
  expect(inBounds({x: 6, y: 10}, bounds, noPadding)).toBe(false);
  expect(inBounds({x: 5, y: 11}, bounds, noPadding)).toBe(false);
})
...
test('chaikinSmoothing', () => {
  const result = chaikinSmoothing([{x: 0, y: 1}, {x: 1, y: 1}, {x: 1, y: 0}])
